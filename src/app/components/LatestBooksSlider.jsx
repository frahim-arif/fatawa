...
export default function LatestBooksSlider() {
  ...
  const getCover = (book) => {
    ...
    let fileId = "";
    if (book.pdf?.includes("/d/")) fileId = book.pdf.split("/d/")[1]?.split("/")[0];
    else if (book.pdf?.includes("id=")) fileId = new URLSearchParams(book.pdf.split("?")[1]).get("id");
  ...
  return (
    <section className="w-full px-2 pb-6 mt-6">
      <div className="rounded-2xl border border-yellow-600/40 bg-white/95 shadow-lg p-3">
