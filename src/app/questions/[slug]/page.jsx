...
async function getRelated() {
  try {
    const res = await fetch(
      `https://f-backend-vdi1.onrender.com/api/admin/questions?limit=10`,
      {
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    twitter: {
      ...
      title,
      description,
      images: ["https://www.maslakedeoband.in/og-image.jpg"],
    },
  };
}
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-6 py-6 space-y-6 text-right">
        ...
        <div className="p-5 md:p-6 rounded-2xl border bg-gray-100 shadow-sm leading-9">
          <p
            ...
            dangerouslySetInnerHTML={{
              __html: autoLink(question.answer, related, slug),
            }}
          />
        </div>
        ...
        <div className="p-5 rounded-2xl border bg-gray-100 space-y-4 text-black">
          {hawalas.map((h, index) => (
            <p key={index} className="arabic text-black leading-8">
              📖 {h}
            </p>
          ))}
        </div>
